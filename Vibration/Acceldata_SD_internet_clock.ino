...
String formatTimeAsCSV(struct tm *timeinfo) {
  char csvTime[30];  // Adjust the size based on your formatting needs
  strftime(csvTime, 30, "%H:%M:%S", timeinfo);
  return String(csvTime);
}
...
void writeAccelerometerDataToCSV() {
  ...
  if(!getLocalTime(&timeinfo)){
    ...
    printLocalTime();
    String csvTimeString = formatTimeAsCSV(&timeinfo);
    ...
    String dataString = csvTimeString + "." + String(milliseconds) + "," +
